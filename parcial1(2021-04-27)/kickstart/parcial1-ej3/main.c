...
int main(int argc, char *argv[]) {
    ...
    bool res = array_is_sorted(playlist, length);
    printf("El arreglo %s está ordenado\n", (res == true) ? "efectivamente" : "no");

    /* check if it is odd-sorted and show message */
    res = array_is_odd_sorted(playlist, length);
    printf("El arreglo %s está ordenado en sus posiciones impares\n", (res == true) ? "efectivamente" : "no");
